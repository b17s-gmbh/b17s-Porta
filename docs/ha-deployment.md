...
`DistributedCacheRefreshLock` uses the same `IDistributedCache` you registered for tickets and sessions - no extra dependency, no extra connection. It implements coordination via a get-then-set with a per-acquire fencing token and compare-and-delete release. `IDistributedCache` does not expose an atomic SET-NX, so the lock is best-effort under contention; for the refresh-coordination use case that narrows the race window to a small fraction of a refresh round-trip, which is enough to eliminate the strict-rotation `invalid_grant` symptom in practice. Consumers who need a true mutex (e.g. RedLock, a SQL row lock) can register their own `IRefreshLock` and the auto-pick steps aside.
